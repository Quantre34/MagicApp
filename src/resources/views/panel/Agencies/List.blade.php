                  ...
                  <table id="file_export" class="table w-100 table-bordered display text-nowrap datatable">
                    ...
                    <tbody>
                      @foreach($Agencies as $Agency)
                        <tr>
                          <td> <img style="width: 50px;height: 50px;" src="{{$Agency['Logo']??'assets/upload/default-logo.png'}}"> </td>
                          <td>{{$Agency['Title']}}</td>
                          <td>{{$Agency['Mail']}}</td>
                          <td>{{$Agency['Tell']}}</td>
                          <td>{{$Agency['CommissionRate']}} %</td>
                          <td class="hidden-xs" width="150">
                              <span class="badge  bg-<?=  $Agency['Status']=='1'? 'success' : 'danger' ?>-subtle text-<?= $Agency['Status']=='1'? 'success' : 'danger' ?>"><?= $Agency['Status']=='1'? 'Aktif' : 'Pasif' ?></span>
                          </td>
                          <td>
                            <div class="box">
                               <a href="/panel/agencies/{{$Agency['uid']}}">
