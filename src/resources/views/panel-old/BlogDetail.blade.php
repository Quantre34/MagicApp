    ...
    <div class="container">
      ...
      <div class="row gx-5">
        <div class="col-xl-8">
          ...
          <h2 class="small-title">Blog Information</h2>
          <div class="card mb-5">
            <div class="card-body">
              
                <form id="FileUploadForm" action="ajax" method="POST" target="UploadFile">                
                <div class="mb-3 row">
                  <label class="col-lg-2 col-md-3 col-sm-4 col-form-label">Blog İmage</label>
          ...
          success: function(data){
            if (data['outcome']) {
                ...
                Object.entries(data['data']).forEach( Item => {
                    ...
                    console.log(value);
                    $('.ArticleList').append('<div onclick="SetArticleInfo(\''+value['uid']+'\')" style="border:  1px solid black; border-radius: 30px; " class="mb-4"><center><div class="col-auto"><div class="sw-5 me-3"><img src="'+(value['Img']??'{{URL::asset('assets/upload/Default.png')}}')+'" class="img-fluid rounded-xl" alt="thumb" /></div></div></center><div  class="text-primary mb-1">'+value['Title']+'</div><div class="text-muted">Article Fee: '+value['Cost']  +'</div><div class="text-muted">Description: '+value['Content'].substr(0, 30)+'...</div><div class="text-muted">'+((value['Status']=='1')? 'Active': 'Pasive')+'</div></div>');
                });
